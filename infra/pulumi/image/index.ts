...
const VPS_ARGS: VirtualServerArgs[] = [
  {
    ...
    sshKeys: ["51520910"],
    // FIXME: export into enums
    tags: [VpsTag.Prod, VpsTag.Backend, VpsTag.Web, VpsTag.Ssh],
    index: 0,
  },
];
const virtualServers = VPS_ARGS.map((a) => new VirtualServer(a));

// create one ssh tunnel for each virtual server
const sshVirtualServers = virtualServers.filter((vs) =>
  vs.tags.apply((ts) => ts.includes(VpsTag.Ssh)),
);
const sshTunnels = sshVirtualServers.map((vs) =>
  pulumi.all([vs.ipv4, vs.index]).apply(([ipv4, index]) => {
    const tunnelName = `${VpsTag.Ssh}${index}`;
